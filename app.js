...
function formatRecordLabel(record, index) {
    ...
    return dateText ? `Entry ${index + 1} - ${dateText}` : `Entry ${index + 1}`;
}
...
function updateTable(animateLast = false) {
    ...
    dailyRecords.forEach((record, index) => {
        ...
        tr.innerHTML = `
            <td data-label="#"><span class="mobile-cell-label">Record</span><span class="mobile-cell-value">${index + 1}</span></td>
            <td data-label="Revenue"><span class="mobile-cell-label">Revenue</span><span class="mobile-cell-value">${formatCurr(record.revenue)}</span></td>
            <td data-label="Cost"><span class="mobile-cell-label">Cost</span><span class="mobile-cell-value">${formatCurr(record.cost)}</span></td>
            <td data-label="Quantity"><span class="mobile-cell-label">Quantity</span><span class="mobile-cell-value">${record.quantity}</span></td>
            <td data-label="Avg price"><span class="mobile-cell-label">Avg price</span><span class="mobile-cell-value">${formatCurr(record.avgPrice)}</span></td>
            <td data-label="Unit cost"><span class="mobile-cell-label">Unit cost</span><span class="mobile-cell-value">${formatCurr(record.avgCost)}</span></td>
            <td data-label="Profit" style="color:${record.profit >= 0 ? 'var(--success)' : 'var(--danger)'}"><span class="mobile-cell-label">Profit</span><span class="mobile-cell-value">${formatCurr(record.profit)}</span></td>
            <td data-label="Actions" class="actions-cell">
                <span class="mobile-cell-label">Actions</span>
    ...
    updateRecordsPicker(animateLast ? dailyRecords.length - 1 : undefined);
}

function updateRecordsPicker(preferredIndex) {
    const select = document.getElementById('record-select');
    const detail = document.getElementById('record-detail-grid');
    ...
    const selectedIndex = Math.min(Math.max(fallbackIndex, 0), Math.max(dailyRecords.length - 1, 0));

    select.innerHTML = '';

    if (dailyRecords.length === 0) {
        select.disabled = true;
        detail.innerHTML = `<p class="record-detail-empty">No data yet - add your first batch above</p>`;
        return;
    }
    ...
    dailyRecords.forEach((record, index) => {
        ...
        option.textContent = formatRecordLabel(record, index);
        select.appendChild(option);
    });

    select.value = String(selectedIndex);
    renderRecordDetail(selectedIndex);
}

function renderRecordDetail(index) {
    const detail = document.getElementById('record-detail-grid');
    if (!detail) return;
    ...
    detail.innerHTML = `
        <div class="record-detail-item"><span>Revenue</span><strong>${formatCurr(record.revenue)}</strong></div>
        <div class="record-detail-item"><span>Cost</span><strong>${formatCurr(record.cost)}</strong></div>
        <div class="record-detail-item"><span>Quantity</span><strong>${record.quantity}</strong></div>
        <div class="record-detail-item"><span>Avg price</span><strong>${formatCurr(record.avgPrice)}</strong></div>
        <div class="record-detail-item"><span>Unit cost</span><strong>${formatCurr(record.avgCost)}</strong></div>
        <div class="record-detail-item"><span>Profit</span><strong style="color:${record.profit >= 0 ? 'var(--success)' : 'var(--danger)'}">${formatCurr(record.profit)}</strong></div>
        <div class="record-detail-actions">
            <button type="button" class="btn-link mobile-edit-record-btn" data-index="${index}">Edit</button>
            <button type="button" class="btn-link mobile-delete-record-btn" data-index="${index}">Delete</button>
        </div>
    `;

    detail.querySelector('.mobile-edit-record-btn')?.addEventListener('click', () => editRecord(index));
    detail.querySelector('.mobile-delete-record-btn')?.addEventListener('click', () => deleteRecord(index));
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (recordSelect) {
        recordSelect.addEventListener('change', () => {
            renderRecordDetail(Number.parseInt(recordSelect.value, 10));
        });
    }
